...
export default function RecentCommits() {
  ...
  const fetchGithubCommits = async () => {
    ...
    try {
      ...
      setCommits(resolvedCommits.slice(0, 3));
    } catch (err) {
  ...
  return (
    <section className={`pt-6 pb-8 transition-colors duration-300 ${isDark ? 'border-t border-zinc-900' : 'border-t border-zinc-200'
      }`}>
      {/* Section Header */}
      <div className="flex items-center justify-between mb-6">
        <h2 className={`text-xl font-bold tracking-tight flex items-center gap-2 transition-colors duration-300 ${isDark ? 'text-white' : 'text-zinc-900'
          }`}>
        ...
        <button
          ...
          disabled={loading}
          className={`px-2.5 py-1 rounded-md transition-all duration-200 cursor-pointer flex items-center gap-1.5 text-xs font-mono-code ${isDark
      ...
      <div className="relative pl-1 sm:pl-2">
        {loading ? (
          ...
          <div className={`py-4 text-xs font-mono-code text-center ${isDark ? 'text-zinc-400' : 'text-zinc-600'
            }`}>
            <p>{error}</p>
            <button
              onClick={fetchGithubCommits}
          ...
          <p className={`text-xs font-mono-code text-center py-4 ${isDark ? 'text-zinc-500' : 'text-zinc-400'
            }`}>
                ...
                <div className={`p-2.5 rounded-lg transition-colors duration-200 ${
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-1 sm:gap-4">
                    ...
                    <a
                      ...
                      className={`inline-flex items-center hover:underline gap-1 text-xs font-mono-code flex-shrink-0 transition-colors ${
                        isDark 
                  ...
                  <div className="flex items-center gap-2 mt-1.5 text-[11px] flex-wrap">
                    <span className={`flex items-center gap-1 ${
                      ...
                      <GitBranch className="w-3 h-3 text-gray-400 opacity-80" />
                      {item.repo}
                    </span>
